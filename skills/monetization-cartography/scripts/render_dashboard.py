#!/usr/bin/env python3
"""Render Monetization Cartography JSON into the bundled HTML template."""
...
def render_category_bars(categories: dict[str, dict[str, Any]]) -> str:
    ...
    for key, category in categories.items():
        ...
        files = category.get("evidence", {}).get("files", [])
        sub = f"{category.get('evidence', {}).get('keyword_hits', 0)} signal hits"
        if files:
            sub += f" · {', '.join(files[:2])}"
        rows.append(
            '<div class="bar-row">'
...
def render_actions(actions: list[dict[str, Any]]) -> str:
    ...
    if not rows:
        rows.append('<tr><td colspan="3">No monetization actions generated.</td></tr>')
...
def evidence_notes(report: dict[str, Any]) -> str:
    ...
    notes = [
        ...
        f"Revenue metric hints found: {len(extras.get('metric_hits', []))}; evidence files: {len(extras.get('evidence_files', []))}.",
        f"Score mode: {meta.get('score_mode', 'heuristic baseline')}. Treat this as monetization review, not financial advice.",
    ]
...
def render(report: dict[str, Any], template: str) -> str:
    ...
    summary = " ".join(insights) or "Heuristic monetization baseline generated from available project evidence."
    replacements = {
