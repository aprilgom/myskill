#!/usr/bin/env python3
"""Codex-Readiness Cartography — repo scorer (100 points, 7 categories).

Audits a repository against the 7-category Codex-Ready rubric and emits structured
...
RE_VERIFY_CMD = re.compile(r"\b(test|lint|typecheck|build|pytest|vitest|jest|playwright|ruff|mypy|tsc|cargo test|go test)\b", re.IGNORECASE)
RE_SAFETY = re.compile(r"\b(secret|env|credential|generated|vendor|migration|do not|don't|never|destructive|reset --hard|checkout --|dirty worktree)\b|비밀|환경변수|생성된|마이그레이션|금지|주의", re.IGNORECASE)
...
def score_a(modules: list[Module], root_context: Path | None) -> CategoryScore:
    ...
    return CategoryScore(
        name="Codex Navigation & Scope Coverage",
        score=pts,
        max=15,
        evidence={
            "core_modules": total,
...
def score_b(context_files: list[Path], repo: Path) -> CategoryScore:
    if not context_files:
        return CategoryScore(name="AGENTS.md / Context Quality", score=0, max=20,
    ...
    if sub["B2_QuickCommands"] < 3:
        findings.append("test/lint/build/typecheck quick command 보강 필요")
    ...
    return CategoryScore(
        name="AGENTS.md / Context Quality",
        score=pts,
        max=20,
        evidence={
            "context_files": n,
...
def score_c(modules: list[Module], repo: Path) -> CategoryScore:
    if not modules:
        return CategoryScore(name="Local Development Workflow Clarity", score=0, max=15)
    ...
    for m in modules:
        ...
        if RE_PURPOSE_HEADING.search(text) or "entry" in lower or "start" in lower:
            q_pass[0] += 1
        if "install" in lower or "setup" in lower or "bootstrap" in lower or "pnpm install" in lower or "pip install" in lower:
            q_pass[1] += 1
        if RE_VERIFY_CMD.search(text):
            q_pass[2] += 1
        if "known fail" in lower or "slow test" in lower or "flaky" in lower or "주의" in text:
            q_pass[3] += 1
    ...
    findings: list[str] = []
    if q_pass[2] < n / 2:
        findings.append("변경 후 실행할 test/lint/build/typecheck 명령이 절반 이상 module에서 누락")
    if q_pass[1] < n / 2:
        findings.append("setup/install/bootstrap 절차가 충분히 문서화되지 않음")
    if q_pass[4] < n / 2:
        findings.append("Codex 작업 완료 기준과 verification evidence 규칙이 부족")

    return CategoryScore(
        name="Local Development Workflow Clarity",
        score=pts,
        max=15,
        evidence={
            "modules_total": n,
...
def score_e(repo: Path, context_files: list[Path]) -> CategoryScore:
    ...
    # E2 Review / completion evidence infra
    ...
    if not has_verify_in_context:
        findings.append("AGENTS.md/context에 local verification command 명시 부족")

    return CategoryScore(
        name="Verification Gates & Testability",
        score=pts,
        max=20,
        evidence={
...
def score_f(modules: list[Module], repo: Path) -> CategoryScore:
    ...
    has_gitignore_env = (repo / ".gitignore").exists() and re.search(r"\.env|secrets?", read_text(repo / ".gitignore"), re.IGNORECASE)
    has_generated_hint = any(re.search(r"generated|do not edit|vendored|vendor|migration", read_text(p), re.IGNORECASE) for p in context_files if p)

    pts = 0
    if context_files:
        pts += round(4 * safety_hits / len(context_files))
    if has_env_example:
        pts += 2
    if has_gitignore_env:
        pts += 2
    ...
    findings: list[str] = []
    if not safety_hits:
        findings.append("AGENTS.md/context에 secrets, generated files, migrations, destructive command 경계가 부족")
    if not has_env_example:
        findings.append(".env.example / .env.sample 부재 — 환경변수 계약이 불명확")
    if not has_gitignore_env:
        findings.append(".gitignore에 .env/secrets 보호 규칙 확인 필요")

    return CategoryScore(
        name="Safety, Secrets & Change Boundaries",
        score=pts,
        max=10,
        evidence={
            ...
            "gitignore_env": bool(has_gitignore_env),
...
def derive_actions(report_partial: dict[str, CategoryScore], modules: list[Module],
    ...
    if C.score < 9:
        actions.append(Action(
            title="AGENTS.md에 setup, test/lint/build, done criteria 보강",
            category="C",
            effort="S", effort_hours=1.0,
            impact="수정 후 검증 루프 명확화 → 미검증 완료 보고 감소",
            impact_score=8,
            priority=8 / 1.0,
        ))
    ...
    if F.score < 6:
        actions.append(Action(
            title="AGENTS.md에 secrets, generated files, migrations, destructive command 경계 추가",
            category="F",
            effort="S", effort_hours=1.0,
            impact="사용자 변경 보존과 민감정보 사고 방지",
            impact_score=8,
            priority=8 / 1.0,
        ))
    ...
    if G.score < 3:
        actions.append(Action(
            ...
            impact="Codex 작업 회귀 측정 가능 — 개선 ROI 자체를 정량화",
            impact_score=6,
            priority=6 / 6.0,
        ))
...
def generate_insights(cats: dict[str, CategoryScore], total: int) -> list[str]:
    ...
    f = cats.get("F")
    if f and f.score < 6:
        out.append("Codex safety boundary 약함 — secrets/generated/destructive command 규칙 보강 필요")
