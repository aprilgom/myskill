...
func installGitHook(repoRoot string) error {
	...
	content := readHookTemplate("githooks/pre-commit", defaultPreCommitHook)
...
func installCodexHook(path string) error {
	...
	hook := readHookTemplate("codex/config.toml", defaultCodexHook)
	data, err := os.ReadFile(path)
	if errors.Is(err, os.ErrNotExist) {
...
func readHookTemplate(relativePath string, fallback string) string {
	if dir := skillDirFromEnvOrExecutable(); dir != "" {
		data, err := os.ReadFile(filepath.Join(dir, "tools", "hooks", relativePath))
		if err == nil {
			return string(data)
		}
	}
...
func skillDirFromEnvOrExecutable() string {
	if dir := os.Getenv("TODO_SKILL_DIR"); dir != "" {
		return dir
	}
...
const defaultPreCommitHook = `#!/usr/bin/env bash
set -euo pipefail
...
const defaultCodexHook = `[features]
hooks = true
