#!/usr/bin/env python3
"""Analyze Codex Desktop session JSONL files for token/context efficiency.

Usage:
  python3 analyze_sessions.py --repo /path/to/repo [--out /tmp/session_analysis.json]
  python3 analyze_sessions.py --sessions-dir ~/.codex/archived_sessions
...
double-counted, then computes a small efficiency rubric.
"""
import argparse
import glob
import json
...
def resolve_sessions_dir(args):
    ...
    return os.path.expanduser("~/.codex/archived_sessions")
...
def repo_filter(files, repo):
    ...
    for fp in files:
        try:
            ...
            if cwd and os.path.abspath(os.path.expanduser(cwd)) == target:
                matched.append(fp)
        except Exception:
            continue
    return matched
...
def main():
    ...
    ap.add_argument("--repo", help="Repository path to filter by session cwd")
    ...
    files = repo_filter(files, args.repo)
    if not files:
