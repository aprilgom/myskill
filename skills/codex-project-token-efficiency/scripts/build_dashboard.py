...
def build_html(data, repo_name):
    ...
    rows = "\n".join(
        f"""
        <tr>
          <td><strong>{html.escape(s.get('display_name') or s.get('thread_name') or s['session_id'][:24])}</strong><div class="muted"><code>{html.escape(s['session_id'][:32])}</code></div><div class="muted">{html.escape(s.get('cwd') or '')}</div></td>
