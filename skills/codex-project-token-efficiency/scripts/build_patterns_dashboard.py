...
def build_html(data):
    ...
    session_rows = "\n".join(
        f"<tr><td><strong>{html.escape(s.get('display_name') or s.get('thread_name') or s['session_id'][:24])}</strong><div class='muted'><code>{html.escape(s['session_id'][:32])}</code></div><div class='muted'>{html.escape(s.get('cwd') or '')}</div></td><td class='num'>{fmt_int(s.get('total_tokens'))}</td><td>{html.escape(', '.join(k for k,p in s['patterns'].items() if p['triggered']) or 'none')}</td></tr>"
...
def open_report(path):
    ...
    if not webbrowser.open(url):
        print(f"[warn] could not open browser automatically: {path}", file=sys.stderr)
        return False
...
def main():
    ...
    ap.add_argument("--no-open", action="store_true", help="Do not open the generated HTML report.")
    args = ap.parse_args()
