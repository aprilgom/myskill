...
determine freshness, stop and report the blocker. Unsupported edits outside
title, evidence, or next check are blocked until the CLI supports them or the
user authorizes a separate non-todo cleanup path. If the request selects a
different action, use the switch-current workflow instead.
